...
{/*
 ...
 返回：待修建卷积核的索引，也就是下一个卷积层待删除的通道数的索引。
 nth_layer: 待修剪的层
 prune_filters：该层需要剪掉的卷积核数量
 */
    if (l->binary) {
        //load_convolutional_weights_binary(l, fp);
        //return;
    ...
    int read_bytes;
    read_bytes = fread(l->biases, sizeof(float), l->n, fp);
    printf("l->batch_normalize:%d,l->dontloadscales:%d\n", l->batch_normalize, l->dontloadscales);
    if (read_bytes > 0 && read_bytes < l->n) printf("\n Warning: Unexpected end of wights-file! l.biases - l.index = %d \n", l->index);
    //fread(l.weights, sizeof(float), num, fp); // as in connected layer
    if (l->batch_normalize && (!l->dontloadscales)) {
        read_bytes = fread(l->scales, sizeof(float), l->n, fp);
    ...
    filter_num = sort_filters(l->weights, l->biases, l->n, prune_filters, size_per_filter);
    ...
    l->nweights = (_msize(l->weights) / sizeof(float));
    l->outputs = l->n*l->out_h * l->out_w;
...
void load_weights_upto_debug(network *net, char *filename, int cutoff, int nth_layer, float percent)
{
    ...
    *net->cur_iteration = get_current_batch(*net);
    ...
    for (i = 0; i < net->n && i < cutoff; ++i) {
        layer l = net->layers[i];
        char* layertype[] = { "CONVOLUTIONAL","DECONVOLUTIONAL","CONNECTED","MAXPOOL","LOCAL_AVGPOOL","SOFTMAX",
            "DETECTION","DROPOUT","CROP","ROUTE","COST","NORMALIZATION","AVGPOOL","LOCAL","SHORTCUT","SCALE_CHANNELS",
            "SAM","ACTIVE","RNN","GRU","LSTM","CONV_LSTM","HISTORY","CRNN","BATCHNORM","NETWORK","XNOR","REGION",
            "YOLO","GAUSSIAN_YOLO","ISEG","REORG","REORG_OLD","UPSAMPLE","LOGXENT","L2NORM","EMPTY","BLANK","CONTRASTIVE","IMPLICIT" };
        printf("%dth layer type: %s\n", i, layertype[l.type]);//打印该层类型layertype[l.type]
        if (l.dontload) continue;
        if (l.type == CONVOLUTIONAL && l.share_layer == NULL && i == nth_layer) {
            ...
            net->layers[i] = l;
            printf("new l.n: %d, l.nweights:  %d\n", net->layers[i].n, net->layers[i].nweights);
            //load_convolutional_weights(l, fp);
        }
        else if (l.type == CONVOLUTIONAL && l.share_layer == NULL && check_next_layer(net, i, nth_layer))
        {
...
//                printf("%d\n", index_array[i]);
        ...
        else if (l.type == CONVOLUTIONAL && l.share_layer == NULL)
        {
            load_convolutional_weights(l, fp);
        }
        if (l.type == SHORTCUT && l.nweights > 0) {
...
void load_weights_debug(network *net, char *filename, char*nth_layer, char*pertentage)
{
    ...
    printf("cut %dth layer percentage: %f\n", n_layer, percent);
