...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "average")){
        ...
        char *filename = (argc > 4) ? argv[4]: 0;//"E:\\speciality\\darknet_v3_prune\\build\darknet\\x64\\cfg\\coco.data"
        test_detector("E:\\speciality\\darknet_v3_debug_prune\\build\\darknet\\x64\\cfg\\coco.data", argv[2], argv[3], filename, thresh, 0.5, 0, ext_output, 0, NULL, 0, 0);
...
int main(int argc, char** argv)
{
    ...
    network net = parse_network_cfg_custom(argv[1], 1,1);    // set batch=1
    if (argv[2]) {//函数参数为指向net的引用，
        load_weights_debug(&net, argv[2], argv[3], argv[4]);
    }
    char buff[256];
    char *backup_directory = "E:\\speciality\\darknet_new_weights";
